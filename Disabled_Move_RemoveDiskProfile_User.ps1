<#
	...
	Производит следующие действия:
		...
		- Учётная запись:
			...
			- удалять все группы безопасности кроме 'Пользователи домена'
...
function ListUsers() {
	...
	$CountYes = 0
	# В переменную 'CountNo' записывается количество НЕ найденных, НЕ удалённых дисков профилей.
	$CountNo = 0
	...
	foreach ($User in $Users) {
		...
		if ($Flag -eq 1) {
			...
			# Очистка переменной $Flag от полученного значения.
	...
	echo " "
	Write-Host " Список удалённых дисков:`n" -ForegroundColor Yellow
 	Write-Host $ListProfileDiskYes
	echo " "
	Write-Host " Список НЕ удалённых, НЕ найденных дисков:`n" -ForegroundColor Yellow
 	Write-Host $ListProfileDiskNo
	...
	$shell.popup("Ура всё получилось", 0, "Результат",  0 + 64 + 4096)
}
