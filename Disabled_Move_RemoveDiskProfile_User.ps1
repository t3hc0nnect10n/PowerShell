...
function ListUsers() {
    ...
    # В цикле while контроль ввода полного пути к места хранения файла списка пользователей.
    while ($true) {
    ...
    # В цикле while контроль ввода полного пути к места хранения дисков профилей в папке
    while ($true) {
    ...
    while($true) {
        ...
        $CheckBlockedUsers = Get-ADObject -LDAPFilter "(objectClass=site)" -SearchBase $BlockedUsers -ErrorAction SilentlyContinue
    ...
    # Переменной $ListUserNameTemp задаём массив (список) для записи ФИО кому принадлежит 'SID'.
    $ListUserNameTemp = [System.Collections.ArrayList]@()
    # Переменной $ListUserSidTemp задаём массив (список) для атрибута 'SID'.
    $ListUserSidTemp = [System.Collections.ArrayList]@()
    # В переменную $ListProfileDisk формируется список найденных дисков.
    ...
    # Цикл foreach производит итерация по учётным записям из списка в заданной переменной $ListUsersFile.
    foreach ($User in $Users) {
        ...
        try {
            ...
            if ($CheckUser) {
                ...
                # Переменной $UserName задаём преобразовать полученный данные из переменной $Name в строку указав двойные кавычки.
                $UserName = "$Name"
                # Убираем лишние символы '@{Name=' и '}', которые получили в виде массива и добавляем в ранее созданный массив (список) в заданной переменной $ListUserNameTemp.
                $ListUserNameTemp.Add($UserName.Replace("@{Name=","").Replace("}",""))    
               
                # Цикл foreach производит итерацию по ФИО из ранее созданного массива (списка) $ListUserName.
                foreach ($UsrName in $ListUserName) {
            
                    # Переменная $CheckUserEnabledFalse проверяет на наличие отключенной учетной записи в организационном подразделении (OU) в заданной переменной $BlockedUsers.
                    $CheckUserNotEnabled = Get-ADUser $User -Filter * -SearchBase $BlockedUsers -Properties * | Where-Object {$_.Enabled -like $false -and $_.Name -like $UsrName}
                    ...
                    if ($CheckUserNotEnabled) {
                        ...
                        Write-Host " Пользователь '$UsrName' -" $User "- заблокирован и перемещен в '$BlockedUsers'. Группы доступа:" $CheckGroup -ForegroundColor Green
                
                        # Переменной $DiskProfileName задаём получить 'SID'.
                        $DiskProfileName =  Get-ADUser $User -Properties *  | Where-Object {$_.Enabled -like $false -and $_.Name -like $UsrName}| Select-Object SID
                        # Переменной $SidName задаём преобразовать полученный данные из переменной $DiskProfileName в строку указав двойные кавычки.
                        $SidName = "$DiskProfileName"
                        # Убираем лишние символы '@{SID=' и '}', которые получили в виде массива и добавляем в ранее созданный массив (список) в заданной переменной $ListUserSidTemp.
                        $ListUserSidTemp.Add($SidName.Replace("@{SID=","*").Replace("}","*"))
                
                        # Цикл foreach производит итерацию по 'SID' из ранее созданного массива (списка) $ListUserTemp.
                        foreach($SID in $ListUserSidTemp) {
                
   				            # Переменной $DiskProfileCheck задаём получить диск профиля в папке 'VM-1C-APP'.
				            $DiskProfileCheck = Get-ChildItem -Path $DiskProfilePath -Name $SID
				            	   
				            if ($DiskProfileCheck) {
					            # Удаление диска профиля.
					            #Remove-Item -Path $DiskProfileCheck
					            Write-Host " Диск профиля '$UsrName': $SID - Удалён" -ForegroundColor Green
					            # В переменную $CountYes плюcуется единица для подсчёта.
					            $CountYes += 1
					            # В массив (список) $ListProfileDiskYes добавляется 'SID'.
					            $ListProfileDiskYes.Add($SID)
					            echo " "
				            }
				            else {
					            Write-Host " Диск профиля '$UsrName': $SID - Не существует" -ForegroundColor Red
					            # В переменную $CountNo плюcуется единица для подсчёта.
					            $CountNo += 1
					            # В массив (список) $ListProfileDiskNo добавляется 'SID'.
					            $ListProfileDiskNo.Add($SID)
					            echo " "
				            }
                        }
            
                        # Очищаем ранее созданный массив (список) $ListUserSidTemp.
                        $ListUserSidTemp.Clear()
                        # Очистка переменной $SidName от заданного значения.
                        Clear-Variable -Name "SidName"            
                    } 
                    else {
                        Write-Host " Пользователь '$UsrName' -" $User "- НЕ заблокирован. Группы доступа:" $CheckGroup -ForegroundColor Red
                    }     
                }
                ...
                echo " "
                Write-Host " Список НЕ удалённых, НЕ найёденных дисков: `n`n$ListProfileDiskNo"
                
                # Очищаем ранее созданный массив (список) $ListUserNameTemp.
                $ListUserNameTemp.Clear()
                ...
                # Очистка переменной $UserName от заданного значения.
...
function OneUser() {
    ...
    while ($true) {
        ...
        $CheckUser = Get-ADOUser -Properties *
    ...
    # Переменной $ListUserNameTemp задаём массив (список) для записи ФИО кому принадлежит 'SID'.
    $ListUserNameTemp = [System.Collections.ArrayList]@()
    # Переменной $ListUserTemp задаём массив (список) для записи 'SID'.
    $ListUserSidTemp = [System.Collections.ArrayList]@()
    
    # Переменной $ADUser задаём получить все свойства учётной записи. 
    $ADUser = Get-ADUser $User -Properties *

    # Перемещение пользователя в организационное подразделение (OU) в заданной переменной $BlockedUsers.
    Get-ADUser $ADUser | Move-ADObject -TargetPath $BlockedUsers

    # Отключение учетной записи.
    Disable-ADAccount -Identity $ADUser

    # Переменная $Groups получает все группы безопасности пользователя кроме "Пользователи домена".
    $Groups = Get-ADPrincipalGroupMembership -Identity $ADUser | Where-Object {$_.Name -ne "Пользователи домена"}
    ...
    # Переменной $ADUserName задаём получить атрибут Name (ФИО) учётной записи. 
    $ADUserName = $ADUser | Select-Object Name
    # Переменной $UserName задаём преобразовать полученный данные из переменной $ADUserName в строку указав двойные кавычки.
    $UserName = "$ADUserName"
    # Убираем лишние символы '@{Name=' и '}', которые получили в виде массива и добавляем в ранее созданный массив (список) в заданной переменной $ListUserNameTemp.
    $ListUserNameTemp.Add($UserName.Replace("@{Name=","").Replace("}",""))

    # Цикл foreach производит итерацию по ФИО из ранее созданного массива (списка) $ListUserNameTemp.
    foreach ($UsrName in $ListUserNameTemp ) {

        # Переменная $Check проверяет на наличие отключенной учетной записи в организационном подразделение (OU) в заданной переменной $BlockedUsers.
        ...
        if ($CheckUser) {
            ...
            $DiskProfileName =  $ADUser | Select-Object SID   
            # Переменной $SidName задаём преобразовать полученный данные из переменной $ADUser в строку указав двойные кавычки.
            $SidName = "$DiskProfileName"
            # Заменяем лишние символы '@{Name=' и '}' на '*', которые получили в виде массива и добавляем в ранее созданный массив (список) в заданной переменной $ListUserSidTemp$.
            $ListUserSidTemp.Add($SidName.Replace("@{SID=","*").Replace("}","*"))

            # Цикл foreach производит итерацию по 'SID' из ранее созданного массива (списка) $ListUserSidTemp.
            foreach($SID in $ListUserSidTemp) {
...
function RemoveProfileDiskDisabledUsers() {
    ...
    Write-Host " Задаём переменной 'FileName' имя файла" -ForegroundColor Yellow
    Write-Host " Пример: Список" -ForegroundColor DarkCyan
    [string]$FileName = Read-Host " Введите имя файла"
    Start-Sleep -Milliseconds 500
    echo " "
    Write-Host " ОК" -ForegroundColor Green
    echo " "
    ...
    # В цикле while контроль ввода полного пути места хранения файла.
    while ($true) {
   
	    Write-Host " Задаём переменной 'FilePath' полный путь к месту хранения файла" -ForegroundColor Yellow
	    Write-Host " Пример: C:\Users\Ivanov\Documents\" -ForegroundColor DarkCyan
	    [string]$FilePath = Read-Host " Введите путь"
	
	    if ($FilePath.Contains("\\") -or $FilePath.Contains("\") -and $FilePath.EndsWith("\")) {

		    $CheckPath = Test-Path -Path $FilePath		    
    ...
    # Переменной $ListUserTemp задаём массив (список) для записи ФИО кому принадлежит 'SID'.
    $ListUserTemp = [System.Collections.ArrayList]@()
    ...
    $GetSid = Get-ADUser -Filter * -SearchBase $BlockedUsers -Properties * | Select-Object SID | Format-Table -HideTableHeaders | Out-File $FilePath$FileName$Extension
    # Переменной $GetFile задаём удалить пустые строки.
    $GetFile = ((Get-Content $FilePath$FileName$Extension) -join [environment]::NewLine).Trim() | Set-Content -Path $FilePath$FileName$Extension
    # Переменной $GetFile задаём удалить лишние символы.
    $GetFile = (Get-Content $FilePath$FileName$Extension).Trim()

    echo " "
    Write-Host "<---------------------------------------------START--------------------------------------------->" -ForegroundColor Red -BackgroundColor White
    echo " "

    Write-Host " Заблокированных учётных записей:" $GetFile.Count -ForegroundColor Yellow 
    echo " "

    # Цикл foreach производит итерацию по каждому элементу из ранее созданного файла в заданной переменной $FileName. 
    foreach ($SID in $GetFile) {
    
        Start-Sleep -Milliseconds 100 

        # Переменной $ADUser задаём получить ФИО по атрибуту 'SID'. 
        $ADUser = Get-ADUser -Filter * -SearchBase $BlockedUsers -Properties * | Where-Object {$_.SID -like $SID} | Select-Object Name
        # Переменной $ADUserName задаём преобразовать полученный данные из переменной $ADUser в строку указав двойные кавычки.
        $ADUserName = "$ADUser"
        # Убираем лишние символы '@{Name=' и '}', которые получили в виде массива и добавляем в ранее созданный массив (список) в заданной переменной $ListUserTemp.
        $ListUserNameTemp.Add($ADUserName.Replace("@{Name=","").Replace("}",""))
    
        # Цикл foreach производит итерацию по каждому пользователю из массива (список) заданной в переменной $ListUserTemp.
        foreach($UsrName in $ListUserNameTemp) {

            # Переменной $SidRename задаём переименовать 'SID' добавив в начало и конец символ * для поиска дисков профилей.
            $SidRename = "*$SID*"
            # Переменной $DiskProfileCheck_VM_1C_APP задаём получить диск профиля.
            $DiskProfileCheck = Get-ChildItem -Path $DiskProfilePath -Name $SidRename
        ...
        # Очищаем ранее созданный массив (список) $ListUserTemp.
        $ListUserNameTemp.Clear()
        # Очищаем переменную $ADUserName от заданных значений.
        Clear-Variable -Name "ADUserName"
    }
    ...
    # Очищаем ранее созданный массив (список) $ListProfileDiskNo.
    $ListProfileDiskNo.Clear()
    # Очищаем переменную $FileName от заданных значений.
    Clear-Variable -Name "FileName"
    # Очищаем переменную $FilePath от заданных значений.
